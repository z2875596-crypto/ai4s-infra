...
- 给出结论或建议
- 使用中文撰写
- 报告中重要的化合物名称、关键数据、结论性语句用 **加粗** 标记
  例如：**LogP = 3.51**、**口服生物利用度高**、**具有良好的 COX-2 选择性**
- 每段正文至少有 1-2 处加粗，突出核心信息

禁止事项（必须严格遵守）：
1. 禁止用 ASCII 字符画图表或雷达图（如 ┌───┐ ═══ │ │ 等），所有数据用文字或表格描述
2. 禁止在报告中插入 Markdown 图片语法，即禁止使用 ![任何文字](任何链接) 格式。SMILES 字符串不需要你处理，前端会自动检测并渲染为分子结构图
3. ADMET 性质评估结果不要用雷达图，改为在报告末尾单独一行输出以下格式的 JSON 数据（数值范围 0-100）：
ADMET_DATA: {{"吸收": 数值, "分布": 数值, "代谢": 数值, "排泄": 数值, "毒性": 数值, "类药性": 数值}}
例如：ADMET_DATA: {{"吸收": 85, "分布": 70, "代谢": 75, "排泄": 80, "毒性": 60, "类药性": 90}}
前端会自动将这段数据渲染为雷达图。如果未预测 ADMET 性质，则不要输出 ADMET_DATA 行。

【严格格式规则】
1. 表格单元格内容必须在同一行，绝对不能换行
2. 禁止使用任何图片语法 ![]()
3. 化学式如 S-(+)-布洛芬 必须写在同一行，不能分行
4. 表格格式必须严格遵守：| 内容 | 内容 | 每行一条记录
5. 禁止在表格内使用 <br>，需要换行时请拆分为多行文本放在表格外
6. 报告最后必须有一个"参考文献"章节，格式如下：
## 参考文献
1. 作者等. 标题. 期刊, 年份. DOI: xxx
2. ...
所有在报告中引用的文献都必须列在这里，包含完整的作者、标题、期刊、年份和 DOI。"""
...
async def _execute_search_literature(args: dict[str, Any]) -> str:
    ...
    for i, p in enumerate(papers, 1):
        ...
        link_line = f"[查看原文 →]({link_url})" if link_url else ""

        lines.append(
            f"**{p.get('title', 'N/A')}**\n"
            f"{authors} | {p.get('year', 'N/A')} | "
            f"{p.get('venue', 'N/A')} | 引用: {p.get('citationCount', 0)}\n"
            f"{link_line}"
        )
    return "\n---\n".join(lines)
